...
class AdminController extends Controller
{
        ...
        if ($fieldType == 'email') {
            $request->validate(
                ...
                [
                    ...
                    'login_id.email' => 'Invalide email address',
                    'login_id.exists' => 'Email is not exists in system',
                    'password.required' => 'Password is required',
                    'password.min' => 'Password must be at least 5 characters',
                    'password.max' => 'Password must be less than 45 characters',
                ]
            );
        } else {
            $request->validate(
    ...
    public function sendPasswordResetLink(Request $request)
    {
        $request->validate(
            ...
            [
                'email.required' => 'Ther :attribute is required',
    ...
    public function resetPassword(Request $request, $token = null)
    {
        ...
        if ($check_token) {
            ...
            if ($diffMins > constDefaults::tokenExpiredMinutes) {
                // If token expired
                session()->flash('fail', 'Token expired, request another reset password');
                return redirect()->route('admin.forgot_password', ['token' => $token]);
            } else {
                return view('back.pages.admin.auth.reset-password')->with(['token' => $token]);
            }
        } else {
            session()->flash('fail', 'Invalid token!, request another reset password');
            return redirect()->route('admin.forgot_password', ['token' => $token]);
        }
    }
    ...
    public function resetPasswordHandler(Request $request)
    {
        $request->validate(
            ...
            [
                ...
                'new_password.max' => 'Password must be less than 45 characters',
                'new_password.same' => 'Password confirmation does not match',
